...
def order_submission():
    ...
    for wafer in data["wafers"]:
        ...
        for chip in wafer["chips"]:
            chip_row = Chips.query.filter_by(
                name=chip["chipId"], wafer_id=wafer_id
            ).first()
            ...
            for lamella in chip["lamellas"]:
                lamella_row = Lamellas(
                    name=lamella["lamellaName"], chip_id=chip_id, status_id=1
                )
...
@orders_api.get("/active_orders")
def active_orders():
    # Create object to hold order data
    order_list = {}

    # Query all orders/lamella data
    rows = OrderLamella.query.options(
        joinedload(OrderLamella.lamella)
        .joinedload(Lamellas.chip)
        .joinedload(Chips.wafer)
    ).all()
    return rows
