...
class CompanyAccounts(db.Model):
    ...
    # Foreign Keys
    status_id = db.Column(
        db.Integer, db.ForeignKey("account_status.id"), nullable=False
    )
...
class Orders(db.Model):
    ...
    contact_id = db.Column(
        db.Integer, db.ForeignKey("company_contacts.id"), nullable=False
    )
    priority_id = db.Column(
        db.Integer, db.ForeignKey("order_priority.id"), nullable=True
    )
    ...
    def to_dict(self):
        return {"id": self.id, "sub_date": self.submission_date}
...
class Chips(db.Model):
    ...
    def to_dict(self):
        return {"id": self.id, "name": self.name, "wafer_id": self.wafer_id}
...
class OrderLamella(db.Model):
    ...
    # Relationships
    order = db.relationship("Orders", backref="order_lamella")
    lamella = db.relationship("Lamellas", backref="order_lamella")

    def to_dict(self):
        return {
            "order_id": self.order_id,
            "wafer_id": self.lamella.chip.wafer.id,
            "wafer_name": self.lamella.chip.wafer.name,
        }
